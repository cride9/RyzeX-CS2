...
using System.Text;
using static SDK.dependencies.memory.Imps;
    ...
    public partial class Mem
    {
        ...
        public nuint VirtualQueryEx(nint hProcess, nuint lpAddress, out MEMORY_BASIC_INFORMATION lpBuffer)
        {
            ...
            if (mProc.Is64Bit || nint.Size == 8)
            {
                ...
                retVal = Native_VirtualQueryEx(hProcess, lpAddress, out tmp64, new nuint((uint)Marshal.SizeOf(tmp64)));
            ...
            retVal = Native_VirtualQueryEx(hProcess, lpAddress, out tmp32, new nuint((uint)Marshal.SizeOf(tmp32)));
        ...
        public nuint GetCode(string name, string path = "", int size = 8)
        {
            ...
            if (!theCode.Contains("+") && !theCode.Contains(","))
            {
                try
                {
                    return new nuint(Convert.ToUInt32(theCode, 16));
                }
                catch
                {
            ...
            if (newOffsets.Contains(','))
            {
                ...
                if (theCode.Contains("base") || theCode.Contains("main"))
                    ReadProcessMemory(mProc.Handle, (nuint)((int)mProc.MainModule.BaseAddress + offsets[0]), memoryAddress, (nuint)size, nint.Zero);
                else if (!theCode.Contains("base") && !theCode.Contains("main") && theCode.Contains("+"))
                {
                    ...
                    if (!moduleName[0].ToLower().Contains(".dll") && !moduleName[0].ToLower().Contains(".exe") && !moduleName[0].ToLower().Contains(".bin"))
                    {
                        ...
                        altModule = int.Parse(theAddr, NumberStyles.HexNumber);
                    }
                    else
                    {
                        try
                    ...
                    ReadProcessMemory(mProc.Handle, (nuint)((int)altModule + offsets[0]), memoryAddress, (nuint)size, nint.Zero);
                }
                else
                ...
                {
                    base1 = new nuint(Convert.ToUInt32(num1 + offsets[i]));
        ...
        public nuint Get64BitCode(string name, string path = "", int size = 16)
        {
            ...
            if (!theCode.Contains("+") && !theCode.Contains(","))
            {
                try
                {
                    return new nuint(Convert.ToUInt64(theCode, 16));
                }
                catch
                {
                    Console.WriteLine("Error in GetCode(). Failed to read address " + theCode);
            ...
            if (newOffsets.Contains(','))
            {
                ...
                if (theCode.Contains("base") || theCode.Contains("main"))
                    ReadProcessMemory(mProc.Handle, (nuint)((long)mProc.MainModule.BaseAddress + offsets[0]), memoryAddress, (nuint)size, nint.Zero);
                else if (!theCode.Contains("base") && !theCode.Contains("main") && theCode.Contains("+"))
                {
                    string[] moduleName = theCode.Split('+');
                    nint altModule = nint.Zero;
                    ...
                    ReadProcessMemory(mProc.Handle, (nuint)(altModule + offsets[0]), memoryAddress, (nuint)size, nint.Zero);
                }
                else // no offsets
                    ReadProcessMemory(mProc.Handle, (nuint)offsets[0], memoryAddress, (nuint)size, nint.Zero);
                ...
                for (int i = 1; i < offsets.Length; i++)
                {
                    base1 = new nuint(Convert.ToUInt64(num1 + offsets[i]));
                    ReadProcessMemory(mProc.Handle, base1, memoryAddress, (nuint)size, nint.Zero);
            ...
            else
            {
                ...
                else if (!theCode.Contains("base") && !theCode.Contains("main") && theCode.Contains("+"))
                {
                    ...
                    if (!moduleName[0].ToLower().Contains(".dll") && !moduleName[0].ToLower().Contains(".exe") && !moduleName[0].ToLower().Contains(".bin"))
                    {
                        string theAddr = moduleName[0];
                        if (theAddr.Contains("0x")) theAddr = theAddr.Replace("0x", "");
                        altModule = (nint)long.Parse(theAddr, NumberStyles.HexNumber);
                    }
                    else
                    {
                        try
        ...
        public bool InjectDll(string strDllName)
        {
            ...
            nuint allocMem = VirtualAllocEx(mProc.Handle, (nuint)null, (uint)lenWrite, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

            WriteProcessMemory(mProc.Handle, allocMem, strDllName, (nuint)lenWrite, out bytesout);
            nuint GameProc = GetProcAddress(GetModuleHandle("kernel32.dll"), "LoadLibraryA");

            if (GameProc == null)
                return false;

            nint hThread = CreateRemoteThread(mProc.Handle, (nint)null, 0, GameProc, allocMem, 0, out bytesout);
            ...
            VirtualFreeEx(mProc.Handle, allocMem, (nuint)0, 0x8000);
        ...
        public nuint CreateCodeCave(string code, byte[] newBytes, int replaceCount, int size = 0x1000, string file = "")
        {
            if (replaceCount < 5)
            ...
            for (var i = 0; i < 10 && caveAddress == nuint.Zero; i++)
            {
                ...
                if (caveAddress == nuint.Zero)
                    prefered = nuint.Add(prefered, 0x10000);
            }

            // Failed to allocate memory around the address we wanted let windows handle it and hope for the best?
            if (caveAddress == nuint.Zero)
                caveAddress = VirtualAllocEx(mProc.Handle, nuint.Zero, (uint)size, MEM_COMMIT | MEM_RESERVE,
        ...
        private nuint FindFreeBlockForRegion(nuint baseAddress, uint size)
        {
            ...
            while (VirtualQueryEx(mProc.Handle, current, out mbi).ToUInt64() != 0)
            {
                ...
                previous = current;
                current = new nuint((ulong)mbi.BaseAddress + (ulong)mbi.RegionSize);
        ...
        public static void SuspendProcess(int pid)
        {
            ...
            {
                nint pOpenThread = OpenThread(ThreadAccess.SUSPEND_RESUME, false, (uint)pT.Id);
        ...
        public bool DumpMemory(string file = "dump.dmp")
        {
            ...
            long proc_min_address_l = (long)proc_min_address; //(Int64)procs.MainModule.BaseAddress;
            long proc_max_address_l = (long)mProc.Process.VirtualMemorySize64 + proc_min_address_l;
            ...
            while (proc_min_address_l < proc_max_address_l)
            {
                ...
                nuint test = (nuint)(long)memInfo.RegionSize;
                nuint test2 = (nuint)(long)memInfo.BaseAddress;
                ...
                proc_min_address = new nuint((ulong)proc_min_address_l);
            }
        ...
        public bool SuspendThreadByID(int ThreadID)
        {
            foreach (ProcessThread thd in mProc.Process.Threads)
            {
                ...
                nint threadHandle = OpenThread(ThreadAccess.SUSPEND_RESUME, false, (uint)ThreadID);
